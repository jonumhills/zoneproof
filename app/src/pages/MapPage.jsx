...
function ParcelPanel({ pin, onClose, onParcelLoaded, mapRef }) {
  ...
  const downloadReport = async () => {
    ...
    try {
      ...
      doc.text('ETHGlobal NYC 2026', PW - M, 64, { align: 'right' });
      ...
      for (let p = 1; p <= totalPages; p++) {
        ...
        doc.setFontSize(7.5);
        doc.setTextColor(...MUTED);
        doc.text('Generated by Townhall  |  ETHGlobal NYC 2026  |  Chainlink CRE + Hedera Hashgraph', M, PH - 10);
        doc.text(`Page ${p} / ${totalPages}`, PW - M, PH - 10, { align: 'right' });
      }

      doc.save(`Townhall-${pin}-Report.pdf`);
    } catch (err) {
